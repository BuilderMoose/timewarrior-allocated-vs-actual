...
def main():
    ...
    report_start = datetime.strptime(report_start_str, '%Y%m%dT%H%M%SZ')
    report_end = datetime.strptime(get_config_val(config_lines, "temp.report.end"), '%Y%m%dT%H%M%SZ')
    
    # 4. Load Allocation File
    ...
    hpd = float(get_config_val(config_lines, "totals.hours_per_day") or 8.0)
    holidays = {line.split('.')[2].split(':')[0] for line in config_lines if "holidays." in line}
    
    # Simple exclusion parser for this example
    exclusions = {}
    for day in ['monday','tuesday','wednesday','thursday','friday']:
        for line in config_lines:
            if f"exclusions.{day}" in line and ">" in line:
                # Basic 9-80 logic check
    ...
    header = f"{'Project':<20} {'Goal':<10} {'Actual':<10} {'Remaining':<10} {'Status'}"
    print(header)
    print("-" * len(header))
